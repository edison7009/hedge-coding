...
每个用过 AI 助手的开发者都撞过这堵墙:

> *"代码本来跑得好好的。我让 AI 优化某一个模块,它把它觉得'冗余'的关键代码删了 —— 因为它根本看不到另外 9 个依赖这段代码的模块。"*

这不是 AI 笨,这是**架构性失败**:

- Agent 在边走边查的过程中,工具返回不断挤占上下文
- 等到它真要改代码时,**全局拓扑认知早就被冲走了**
- 局部优化在没有全局视野的情况下盲目执行

**Hedge Coding 在架构层根治这个问题:**

- Tree-sitter 把**每一个模块**的签名抽出来生成 Repo Map,紧凑到能在整个任务过程中一直驻留
- Super Prompt 是**预编译的完整上下文块**,一次性递给 Opus/GPT,中途不被打断
